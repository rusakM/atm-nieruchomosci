...
function edit_offer($con, $id) {
    ...
    $s=$s.'<form action="http://localhost/atm-nieruchomosci/scripts/edit.php?id='.$id.'" method="post">
        <div class="form-group">
            <input type="checkbox" value="0" id="isEditing-title" onchange="switchInputEnable(event);">
            <label for="title">Tytuł</label>
            <input type="text" id="title" class="form-control" name="title" value="'.$offer['title'].'" disabled>
        </div>
        <div class="form-group">
            <input type="checkbox" value="0" id="isEditing-price" onchange="switchInputEnable(event);"> 
            <label for="price">Cena:</label>
            <input type="number" name="price" value="'.$offer['price'].'" id="price" class="form-control" disabled>
        </div>
        <div class="form-group">
            <input type="checkbox" value="0" id="idEditing-area" onchange="switchInputEnable(event);">
            <label for="area">Powierzchnia:</label>
            <input type="number" id="area" name="area" class="form-control" value="'.$offer['area'].'" disabled>
        </div>
        <div class="form-group">
            <input type="checkbox" value="0" id="isEditing-cityId" onchange="switchInputEnable(event);">
    ...
    $s = $s.'</select></div>
        <div class="form-group">
            <input type="checkbox" value="0" id="isEditing-typeOfEstate" onChange="switchInputEnable(event);">
    ...
    $s = $s.'</select></div>
        <div class="form-group">
            <input type="checkbox" value="0" id="isEditing-typeOfMarket" onchange="switchInputEnable(event);">
        ...
        <div class="form-group">
            <input type="checkbox" id="isEditing-typeOfTransaction" onchange="switchInputEnable(event);">
        ...
        <div class="form-group">
            <input type="checkbox" id="isEditing-adress" onchange="switchInputEnable(event);">
            <label for="adress">Adres</label>
            <input type="text" value="'.$offer['adress'].'" id="adress" name="adress" class="form-control" disabled>
        </div>
        <div class="form-group">
            <input type="checkbox" id="isEditing-description" onchange="switchInputEnable(event);">
            <label for="description">Opis</label>
            <textarea id="description" name="description" class="form-control" disabled rows="15">'.$offer['description'].'</textarea>
        </div>
        <div class="form-group">
            <input type="checkbox" id="isEditing-isFinished" onchange="switchInputEnable(event);">
            <label for="isFinished">Wykończone:</label>
        ...
        <div class="form-group">
            <input type="checkbox" id="isEditing-isFurnitured" onchange="switchInputEnable(event);">
        ...
        <div class="form-group">
            <input type="checkbox" value="0" id="idEditing-numOfRooms" onchange="switchInputEnable(event);">
        ...
        <div class="form-group">
            <input type="checkbox" value="0" id="idEditing-floor" onchange="switchInputEnable(event);">
            <label for="floor">Piętro:</label>
    ...
    foreach($photos as $photo) {
        ...
        if($photo[2] == 0) {
            $s = $s.'<a href="http://localhost/atm-nieruchomosci/scripts/set_as_primary.php?id='.$photo[0].'&offer='.$id.'">
    ...
    <div class="new-photos">
        <h3>Nowe zdjęcia:</h3>
        <form action="http://localhost/atm-nieruchomosci/scripts/add_photo.php?id='.$id.'" method="post" enctype="multipart/form-data">
            <div id="photos-container">
                <div class="photo-container">
                    <div class="photo-preview" id="photo-preview-1"></div>
                    <input type="file" name="photo-1" id="photo-1" onchange="fileChangeHandler(event);" accept="image/jpeg">
